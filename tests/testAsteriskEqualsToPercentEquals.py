...
class MutatorTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.m = all_mutants["ArithmeticMultEqualsToModEquals"]

    def test_StringWithDoubleAsterisk__ReturnsUnchanged(self):
        str = "100 **= 90 **= 80 **= 70"
        self.assertEqual(str, self.m.mutate(str))

    def test_StringWithAsteriskEqual_ReturnsWithPercentEqual(self):
        input_str = "6 * 7 *= 9 *== 10"
        final_str = "6 * 7 %= 9 *== 10"
        self.assertEqual(final_str, self.m.mutate(input_str))

    def test_GetName(self):
        self.assertEqual("ArithmeticMultEqualsToModEquals", self.m.get_name())
